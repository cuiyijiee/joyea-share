...
class SrcCollectHandler extends IAction {
  override def execute(request: JsonObject, listener: ExecListener): Unit = {
    ...
    method match {
      ...
      case "pageFind" =>
        ...
        SrcCollect.pageFindByUserId(
          ...
          if (listTry.isSuccess) {
            ...
            SrcCollect.countByUserId(userId = context.sessiono[String]("user_id").getOrElse("")).onComplete(countTry => {
      ...
      case "delete" =>
        val collectId = request.get("id").asLong()
        SrcCollect.delete(collectId).onComplete(delTry => {
          if (delTry.isSuccess) {
            listener.onSuccess(respJson = resJson)
          } else {
            listener.onError("删除收藏资源失败:" + SUtil.convertExceptionToStr(delTry.failed.get))
          }
        })
      case _ =>
    }
