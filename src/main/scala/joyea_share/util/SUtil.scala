...
object SUtil {
  ...
  def genYesterdayDateString(formatString: String = "yy-MM-dd HH:mm:ss"): String = {
    val yesterdayDate = new Date(new Date().getTime() - 24 * 60 * 60 * 1000)
    new SimpleDateFormat(formatString).format(yesterdayDate)
  }
