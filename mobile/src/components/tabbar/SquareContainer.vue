                        ...
                        <template #footer>
                            <div style="margin-top: 5px">
                                ...
                                <van-tag mark type="warning" style="margin-right: 2px;" v-if="item.hangyeTagId !== -1"
                                         color="#cccccc">
                                    {{
                                        hangyeOption.find(option => {
                                            return option.value === item.hangyeTagId
                                        }).text
                                    }}
                                </van-tag>
                                <van-tag mark type="warning" style="margin-right: 2px" v-if="item.xianbieTagId !== -1"
                                         color="#cccccc">
                                    {{
                                        xianbieOption.find(option => {
                                            return option.value === item.xianbieTagId
                                        }).text
                                    }}
                                </van-tag>
                                <van-tag mark type="warning" style="margin-right: 2px" v-if="item.jixingTagId !== -1"
                                         color="#cccccc">
                                    {{
                                        jixingOption.find(option => {
                                            return option.value === item.jixingTagId
                                        }).text
                                    }}
                                </van-tag>
                                <van-tag mark type="warning" style="margin-right: 2px" v-if="item.jieduanTagId !== -1"
                                         color="#cccccc">
                                    {{
                                        jieduanOption.find(option => {
                                            return option.value === item.jieduanTagId
                                        }).text
                                    }}
                                </van-tag>
                                <van-tag mark type="warning" style="margin-right: 2px" v-if="item.shichangTagId !== -1"
                                         color="#cccccc">
                                    {{
                                        shichangOption.find(option => {
                                            return option.value === item.shichangTagId
                                        }).text
                                    }}
                                </van-tag>
                            </span>
                                <van-button plain size="mini" @click="handleCollect(item)">{{
                                        (item.isCopied ? "已收藏 " :
                                            "收藏 ") + item.likeNum
                                    }}
                                </van-button>
