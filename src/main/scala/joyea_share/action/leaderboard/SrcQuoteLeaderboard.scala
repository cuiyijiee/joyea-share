...
import joyea_share.model.{JoyeaUser, SrcQuote}
import xitrum.annotation.POST

@POST("api/v1/leaderboard/srcQuote")
class SrcQuoteLeaderboard extends BaseAction[LeaderboardReq] {
    override def safeExecute(req: LeaderboardReq): Unit = {
        val dateRange = LeaderboardUtil.generateDateRange(req.year, req.month)
        SrcQuote.findWithDateLimit(dateRange._1, dateRange._2)
